...
class SEIR:
    def __init__(self, graph: nx.Graph):
        ...
        self.removed_list = list()
        self.exposed_list = set()
        self.new_exposed_list = set()
    ...
    def initialize_fixed_HO_structure(self, group_list):
        # self._g2 = {i: {"nodes": list(set(gr)), "size": len(gr), "infected": 0} for i, gr in enumerate(group_list) if
        #             len(gr) >= 3}
        ...
        if self.beta1 > 0:
            for u, v in list(self._graph.edges()):
                if self.node_dict[u] == 1 and self.node_dict[v] == 0 and random.uniform(0, 1) < self.beta1:
                    self.new_exposed_list.add(v)
                    self.healthy_list.remove(v)
                    self.node_dict[v] = 3
                if self.node_dict[u] == 0 and self.node_dict[v] == 1 and random.uniform(0, 1) < self.beta1:
                    self.new_exposed_list.add(u)
                    self.healthy_list.remove(u)
                    self.node_dict[u] = 3
    ...
    def threshold_clique(self):
        if self.beta2 > 0:
            for feature in self._g2.keys():
                if self._g2[feature]["size"] > 2 and self._g2[feature]["infected"] / (
                self._g2[feature]["size"]) >= self.threshold:
        ...
        if self.beta2 > 0:
            for group in self._g2.keys():
                if self._g2[group]["size"] >= 3:
                    triangles = [(node[0], node[1], node[2]) for node in
                                 combinations(list(self._g2[group]["nodes"]), 3)]
                            ...
                            if self.node_dict[n2] == 1:
                                if self.node_dict[n3] == 0:
                                    ...
                                    if random.random() <= self.beta2:
                                        self.new_exposed_list.add(n3)
                                        self.healthy_list.remove(n3)
                                        self.node_dict[n3] = 3
                            else:
                                if self.node_dict[n3] == 1 and self.node_dict[n2] == 0:
                                    # infect n2 with probability beta2
                                    if random.random() <= self.beta2:
                                        self.new_exposed_list.add(n2)
                                        self.healthy_list.remove(n2)
                                        self.node_dict[n2] = 3
                        else:
                            if self.node_dict[n2] == 1 and self.node_dict[n3] == 1 and self.node_dict[n1] == 0:
                                # infect n1 with probability beta2
                                if random.random() <= self.beta2:
                                    self.new_exposed_list.add(n1)
                                    self.healthy_list.remove(n1)
                                    self.node_dict[n1] = 3

    def update_new_infected(self):
        for node in list(self.exposed_list):
            if random.uniform(0, 1) < self.alpha:
                self.infected_list.append(node)
                self.exposed_list.remove(node)
                self.node_dict[node] = 1
                if len(self.node_association[node]) > 0:
                    for feature in self.node_association[node]:
                        self._g2[feature]["infected"] += 1
        self.exposed_list = self.exposed_list.union(self.new_exposed_list)
        self.new_exposed_list = set()
    ...
    def run(self, numiters):
        ...
        while True:
            ...
            state["num_susceptible"] = len(self.healthy_list)
            state["nodes"] = self.node_dict.copy()
            state["iteration"] = time
            state["num_exposed"] = len(self.exposed_list)
            state["num_removed"] = len(self.removed_list)
            ...
            self._result["trends"]["R"].append(len(self.removed_list) / self._graph.number_of_nodes())
            self._result["trends"]["E"].append(len(self.exposed_list) / self._graph.number_of_nodes())
