...
class IndependentCascadeModel:
    ...
    def set_initial_status(self , configuration : ModelConfig):
        ...
        self._g2 = { i : {"nodes" : [] , "size":0 , "infected": 0 , "threshold":0} for i in range(self._config["number_of_groups"])}
        ...
        for key in self.vertex_set:
            for feature in self._g2.keys():
                if random.uniform(0, 1) < self.int_p:
                    ...
                    self._g2[feature]["threshold"] = random.uniform(0, 1)
    ...
    def initialize_fixed_HO_structure(self , group_list):
        ...
        for i, gr in enumerate(group_list):
            self._g2[i] = {"nodes": list(set(gr)), "size": len(gr), "infected": 0 , "threshold": random.uniform(0, 1)}
            for node in gr:
    ...
    def threshold_clique(self):
        if self.beta2> 0:
            for feature in self._g2.keys():
                if self._g2[feature]["size"]>2 and self._g2[feature]["infected"] / (self._g2[feature]["size"])>=self._g2[feature]["threshold"]:
